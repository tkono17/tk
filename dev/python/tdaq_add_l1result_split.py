...
for event in input:
  if (not output_is_open) or (ievent_in_output_file>=nevents_in_output):
    output_file_name = input_file + '_' + str(i_output_file)
    output = None
    if ievent > 0:
      command = 'mv daq_python_script_0000000_file01.data %s' % output_file_name
      os.system(command)
    output_is_open = False
    ievent_in_output_file = 0
    i_output_file += 1
  if output==None:
    output_file_name = input_file + '_' + str(i_output_file)
    output = EventApps.ostream()
    output_is_open = True
  ievent_in_output_file += 1
